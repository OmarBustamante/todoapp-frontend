...
import { fetchAllTodos, fetchTodosPage } from './services/todoService'

function App() {
  const [allData, setAllData] = useState([])
  const [page, setPage] = useState([])
  // Parámetros para el filtro del fetch
  ...
  const fetchPage = (num: number, text: string, priority: string, done: string, sort: string) => {
    fetchTodosPage(num, text, priority, done, sort)
  ...
  for (let i = 1; i <= numPages; i++) {
    pagination.push(
      <button
        key={i}
        className={`w-5 h-fit m-1 border-1 hover:bg-gray-200 ${num === i ? "bg-gray-300" : ""}`}
        onClick={() => setNum(i)}
      >
        {i}
      </button>
  ...
  return (
    <>
      <div className='mt-10 mx-auto flex flex-col w-[50%]'>
        ...
        <div className='my-5'>
          <button
            className=' bg-green-500 hover:bg-green-700 p-2 border-2'
        ...
        <div className='my-8 text-center'>
          <button
            className={`w-5 h-fit m-1 border-1 hover:bg-gray-200`}
            onClick={() => setNum(1)}
          >
            {"<<"}
          </button>
          ...
          <button
            className={`w-5 h-fit m-1 border-1 hover:bg-gray-200`}
            onClick={() => {
              num !== Math.floor(numPages) ? setNum(num + 1) : setNum(Math.floor(numPages))
            }}
          >
            {">"}
          </button>
          <button
            className={`w-5 h-fit m-1 border-1 hover:bg-gray-200`}
            onClick={() => setNum(Math.floor(numPages))}
          >
            {">>"}
          </button>
        </div>
        <div className='border-2 w-full flex justify-between p-5'>
          <div>
            ...
            <div>
              Total: {contadorTotal === 0 ? formatTime(0) : formatTime(timeTotal / contadorTotal)}
            </div>
          </div>
          <div className=''>
            <h2 className='font-bold'>Average time to finish tasks by priority: </h2>
            <p>
              High: {contadorHigh === 0 ? formatTime(0) : formatTime(timeHigh / contadorHigh)}
            </p>
            <p>
              Medium: {contadorMed === 0 ? formatTime(0) : formatTime(timeMed / contadorMed)}
            </p>
            <p>
              Low: {contadorLow === 0 ? formatTime(0) : formatTime(timeLow / contadorLow)}
            </p>
          </div>
        </div>
        <ModalNew modalOpen={modalOpen} setModalOpen={setModalOpen} setReload={setReload} />
      </div>
    </>
  )
}
